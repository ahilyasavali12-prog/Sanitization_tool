...
CAPABILITY_KEYWORDS = {
    ...
    "credential_access": [
        r"\bpasswords?\b", r"\bapi[_ ]?keys?\b", r"\btokens?\b", r"\bsecrets?\b",
        r"\bcredentials?\b", r"\bssh[_ ]?keys?\b", r"\benv(iron(ment)?)? var(iable)?s?",
        r"\.env\b",
    ],
    "code_exec": [
...
RISKY_COMBINATIONS = [
    ({"credential_access", "data_exfil"},
     "One tool can access credentials/secrets and another can send data "
    ...
    ({"destructive", "credential_access"},
     "One tool can access credentials and another can perform destructive "
     "actions — combined, a compromised credential could be used "
...
def detect_multi_tool_threshold(tools: list) -> list:
    """
    Layer 2: cross-tool correlation.

    Looks across the WHOLE toolset (not one tool at a time) for risky
    capability combinations spread across two or more different tools.
    Returns a list of flagged findings.
    """
    per_tool_caps = {tool.get("name", f"tool_{i}"): classify_capabilities(tool)
                     for i, tool in enumerate(tools)}

    findings = []
    for combo, explanation in RISKY_COMBINATIONS:
        # which tools contribute which piece of this combo?
        ...
        # only flag if EVERY capability in the combo is covered, AND at
        # least two distinct tools are involved (the "threshold" part —
        # a single tool doing both isn't cross-tool poisoning, it's just
        # an obviously risky single tool, which layer 1 or classifier
        # would likely already flag).
        if all(contributors[cap] for cap in combo):
            distinct_tools = set()
...
def sanitize_toolset(tools: list, image_texts: dict = None) -> dict:
    ...
    return {
        ...
        "cross_tool_risk_detected": len(cross_tool_findings) > 0,
    }