...
def print_report(report: dict):
    ...
    if not report["cross_tool_findings"]:
        ...
        for finding in report["cross_tool_findings"]:
            ...
            print(f"[{sev}] {' + '.join(finding['combination'])}")
    ...
    elif any(f["severity"] in ("medium", "low") for f in report["cross_tool_findings"]):
        print("REVIEW RECOMMENDED — no high-confidence issues, but some")
        print("cross-tool combinations are worth a manual look (see above).")
    else:
